...
{\rule{30ex}{0ex}\bf Table 1.} \vspace{+3mm}\\
...
%\renewcommand{\arraystretch}{.7} 
{\rule{15ex}{0ex}\bf Table 2.} \vspace{+3mm}\\
...
%\renewcommand{\arraystretch}{.7} 
{\rule{15ex}{0ex}\bf Table 3.}\vspace{+3mm} \\
\centerline{\begin{tabular}{c|c|c|c|c|c}
 Locus $1\backslash\mbox{Locus}\; 2$       & "$++$"   & "$+-$"  &  "$-+$"   & "$--$" & Total   \\
\hline      
...
{\bf 3: Allelic LDE and maximum likelihood method.}  If we have haplotype data from diploid organism we can easily specify frequencies of allelelic variants in Table 1 from the count table  \\
%\renewcommand{\arraystretch}{.7} 
{\rule{18ex}{0ex}\bf Table 5.}\vspace{+3mm} \\
\centerline{\begin{tabular}{c|c|c|c|c|c}
 Locus $1\backslash\mbox{Locus}\; 2$       & "$++$"   & "$+-$"  &  "$-+$"   & "$--$" & Total   \\
\hline      
"$++$"     & $n_{\ssc ++++}$  & \multicolumn{2}{c|}{$n_{\ssc +++-}$}  & $n_{\ssc ++--}$ & $n_{\ssc ++\bullet}$   \\
\hline  
"$+-$"     & $n_{\ssc +-++}$  & $n_{\ssc +-+-}$ & $n_{\ssc +--+}$ & $n_{\ssc +---}$ & $n_{\ssc +-\bullet}$  \\
\hline  
"$--$"     & $n_{\ssc --++}$  & \multicolumn{2}{c|}{$n_{\ssc --+-}$} & $n_{\ssc ----}$ & $n_{\ssc --\bullet}$  \\
\hline  
Total & $n_{\ssc \bullet ++}$    & \multicolumn{2}{c|}{$n_{\ssc \bullet +-}$} &  $n_{\ssc \bullet --}$   & $n$ \\
\end{tabular}}
Then the corresponding allelic counts are given in the following table
\\
{\rule{1ex}{0ex}\bf Table 6.} \vspace{+3mm}\\
\centerline{\begin{tabular}{c|c|c}
 Locus $1\backslash\mbox{Locus}\; 2$   & "$+$"        & "$-$"   \\
\hline      
"$+$"     & $2n_{\ssc ++++}+n_{\ssc +++-}+n_{\ssc +-++}+n_{\ssc +-+-}$   & $2n_{\ssc ++--}+n_{\ssc +++-}+n_{\ssc +--+}+n_{\ssc +---}$  \\
"$-$"     & $2n_{\ssc --++}+n_{\ssc +-++}+n_{\ssc +--+}+n_{\ssc --+-}$  & $2n_{\ssc ----}+n_{\ssc --+-}+n_{\ssc +-+-}+n_{\ssc +---}$   \\
\end{tabular}} 
and, taking account of the $2n$ total number of gametes, the corresponding frequencies are following:
\\
{\rule{1ex}{0ex}\bf Table 7.} \vspace{+3mm}\\
\centerline{\small{\begin{tabular}{c|c|c}
 Locus $1\backslash\mbox{Locus}\; 2$   & "$+$"        & "$-$"   \\
\hline      
"$+$"     & $p_{\ssc ++++}+p_{\ssc +++-}/2+p_{\ssc +-++}/2+p_{\ssc +-+-}/2$   & $p_{\ssc ++--}+p_{\ssc +++-}/2+p_{\ssc +--+}/2+p_{\ssc +---}/2$  \\
"$-$"     & $p_{\ssc --++}+p_{\ssc +-++}/2+p_{\ssc +--+}/2+p_{\ssc --+-}/2$  & $p_{\ssc ----}+p_{\ssc --+-}/2+p_{\ssc +-+-}/2+p_{\ssc +---}/2$   \\
\end{tabular}} }
\rule{0ex}{1.5em}
In the case of genotypes we have only $n_{\ssc +-+-}+n_{\ssc +--+}$ instead of $n_{\ssc +-+-}$ and $n_{\ssc +--+}$. One can simplify the parametization
\pagebreak

{\rule{1ex}{0ex}\bf Table 8.} \vspace{+3mm}\\
\centerline{\small{\begin{tabular}{c|c|c}
 Locus $1\backslash\mbox{Locus}\; 2$   & "$+$"        & "$-$"   \\
\hline      
"$+$"     & $p_{\ssc ++}=p_{\ssc ++}^*+p_{\ssc +-+-}/2$   & $p_{\ssc +-}=p_{\ssc +-}^*+p_{\ssc +--+}$/2  \\
"$-$"     & $p_{\ssc -+}=p_{\ssc -+}^*+p_{\ssc +--+}/2$  & $p_{\ssc --}^*=p_{\ssc --}+p_{\ssc +-+-}/2$   \\
\end{tabular}} }
\rule{0ex}{1.5em}
and join the observed counts to $n_{\ssc ++}^*,n_{\ssc +-}^*,n_{\ssc -+}^*,n_{\ssc --}^*,n_{\ssc +-+-}^*$ , where 
\begin{eqnarray*}
&& n_{++}^{*}=2n_{\ssc ++++}+n_{\ssc +++-}+n_{\ssc +-++}; \\
&& n_{+-}^{*}=2n_{\ssc ++--}+n_{\ssc +++-}+n_{\ssc +---}; \\
&& n_{-+}^{*}=2n_{\ssc --++}+n_{\ssc +-++}+n_{\ssc --+-}; \\
&& n_{--}^{*}=2n_{\ssc ----}+n_{\ssc --+-}+n_{\ssc +---}; \\
&& n_{\ssc +-+-}^*=2(n_{\ssc +-+-}+n_{\ssc +--+})
\end{eqnarray*}
and $ n_{\ssc ++}^{*}+ n_{\ssc +-}^{*}+n_{\ssc -+}^{*}+ n_{\ssc --}^{*}+n_{\ssc +-+-}^*=2n$. The obtaind statistical model is overparametrized; one can use the follofing relations $p_{\ssc +-+-}=p_{\ssc ++}p_{\ssc --}$ and $p_{\ssc +--+}=p_{\ssc +-}p_{\ssc -+}$ (local Hardy--Weinberg equilibrium in some sense). Then, the log liklihood function in the model can be written as follows:
$$
\begin{array}{rcl}
\di
LL(\vec p^*|\vec n^*)=n_{\ssc ++}^*\log p_{\ssc ++}^*+n_{\ssc +-}^*\log p_{\ssc +-}^*&\!+\!&\di n_{\ssc -+}^*\log p_{\ssc -+}^*+ n_{\ssc --}^*\log p_{\ssc --}^*\\ 
&+& \di
n_{\ssc +-+-}^* \log (p_{\ssc ++}^*p_{\ssc --}^*+p_{\ssc +-}^*p_{\ssc +-}^*). 
\end{array}
\eqno(A)
$$
The maximum likelihood can be obtained using the EM-algorithm based on the full likelihood, which is unobserved:
\begin{eqnarray*}
LL^{\circ}(\vec p|\vec n)&=&n_{\ssc ++}^*\log p_{\ssc ++}^*+n_{\ssc +-}^*\log p_{\ssc +-}^*+ n_{\ssc -+}^*\log p_{\ssc -+}^*+ n_{\ssc --}^*\log p_{\ssc --}^*\\ 
&+& 
2n_{\ssc +-+-} (\log p_{\ssc ++}+\log p_{\ssc --})+2n_{\ssc +-+-} (\log p_{\ssc +-}+\log p_{\ssc +-}). 
\end{eqnarray*}
...
\noindent
{\it Calculate $\delta$(normally, $\delta>0$):} 
$$
...
{\rule{10ex}{0ex} \bf Table 9.} \vspace{+3mm}\\
\centerline{\begin{tabular}{c|c|c|c}
 Locus $1\backslash\mbox{Locus}\; 2$   & "$+$"        & "$-$"       & Total \\
\hline      
"$+$"     & $p_{\ssc  ++}^{(K)}+p_{\ssc  ++}^{(K)}p_{\ssc  --}^{(K)}/2$   & $p_{\ssc  +-}^{(K)}+p_{\ssc  +-}^{(K)}p_{\ssc  -+}^{(K)}/2$   & $p_{\ssc  +\bullet}^{(K)}+p^{(K)}$ \\
"$-$"     & $p_{\ssc  -+}^{(K)}+p_{\ssc  +-}^{(K)}p_{\ssc  -+}^{(K)}/2$   & $p_{\ssc --}^{(K)}+p_{\ssc  ++}^{(K)}p_{\ssc  --}^{(K)}/2$   & $p_{\ssc -\bullet}+p^{(K)}$ \\
\hline
Total & $p_{\ssc \bullet +}+ p^{(K)}$    & $p_{\ssc \bullet -}+p^{(K)}$    & $1$ \\
\end{tabular}}
\rule{0ex}{1.5em}\rule{10ex}{0em}
where $P^{(K)}=\bigl(p_{\ssc  ++}^{(K)}p_{\ssc  --}^{(K)}+p_{\ssc  ++}^{(K)}p_{\ssc  --}^{(K)}\bigr)/2$. 
